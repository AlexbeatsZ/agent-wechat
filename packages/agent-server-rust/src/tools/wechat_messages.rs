...
/// Get the latest localId for a specific chat from the WeChat DB.
///
/// Returns the maximum local_id from the Msg table for the given chat.
/// Used for DB-based send confirmation: compare before/after to detect new messages.
pub fn get_latest_local_id(
    account_dir: &str,
    keys: &HashMap<String, String>,
    chat_id: &str,
) -> Option<i64> {
    ...
    let rows = query_wechat_db(
        &db_path,
        key,
        &format!("SELECT MAX(local_id) as max_id FROM \"{table_name}\";"),
    );
...
/// Get the latest self-sent message localId for a specific chat.
///
/// Filters to messages where the sender matches the logged-in user (account_dir prefix).
/// Returns the maximum local_id among self-sent messages.
pub fn get_latest_self_local_id(
    account_dir: &str,
    keys: &HashMap<String, String>,
    chat_id: &str,
) -> Option<i64> {
    ...
    let rows = query_wechat_db(
        ...
        key,
        &format!(
            "SELECT MAX(m.local_id) as max_id FROM \"{table_name}\" m \
             LEFT JOIN Name2Id n ON m.real_sender_id = n.rowid \
             WHERE n.user_name = '{}';",
            account_dir.replace("'", "''")
        ),
...
/// Poll the WeChat DB for a new self-sent message after sending.
///
/// Checks up to `attempts` times with `interval_ms` delay between each.
/// Returns the new message's local_id if found, or None if not confirmed.
pub async fn poll_db_for_new_self_message(
    account_dir: &str,
    keys: &HashMap<String, String>,
    chat_id: &str,
    ...
    for attempt in 1..=attempts {
        ...
        let latest = get_latest_self_local_id(account_dir, keys, chat_id);
        if let Some(new_id) = latest {
            if new_id > baseline_local_id {
                tracing::info!(
                    "[send-confirm] DB poll attempt {attempt}: confirmed new self msg local_id={new_id} (baseline={baseline_local_id})"
                );
                return Some(new_id);
            }
        }

        // Also check max local_id (self-sent may not be the latest)
        let max = get_latest_local_id(account_dir, keys, chat_id);
        if let Some(max_id) = max {
            if max_id > baseline_local_id {
                tracing::info!(
                    "[send-confirm] DB poll attempt {attempt}: new msg detected max_id={max_id} > baseline={baseline_local_id}, checking self status..."
                );
                // New message exists but not confirmed as self yet - may need more time
                // Continue polling
            }
        }

        tracing::debug!(
            "[send-confirm] DB poll attempt {attempt}/{attempts}: no new self msg yet (baseline={baseline_local_id})"
        );
    }

    tracing::warn!(
        "[send-confirm] DB poll failed after {attempts} attempts: could not confirm new self message for {chat_id}"
    );
    None
}
