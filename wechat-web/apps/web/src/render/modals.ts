...
export function renderModals(): void {
  const el = document.querySelector<HTMLElement>('[data-region="modals"]');
  if (!el) return;
  el.innerHTML = `
    ...
    ${state.previewImageUrl ? `<div class="modal image-preview-modal" data-action="close-preview"><div class="image-preview-panel"><img src="${state.previewImageUrl}" alt="大图预览">${state.previewImageStatus ? `<span class="image-preview-status">${escapeHtml(state.previewImageStatus)}</span>` : ""}</div></div>` : ""}
  `;
}
