...
import type { ChatDto } from "../types.js";
...
function renderServiceFolder(items: ChatDto[]): string {
...
function renderChatItem(item: ChatDto): string {
  return `
...
function avatarText(item: ChatDto): string {
  if (item.kind === "filehelper") return "文";
  if (item.isGroup) return "群";
