export function publicError(code?: string, message?: string): string {
...
export async function responseError(response: Response): Promise<string> {
  const body = await response.json().catch(() => ({})) as { code?: string; error?: string; reason?: string };
  if (body.reason === "original_not_available") return "原图尚未下载到本机微信";
