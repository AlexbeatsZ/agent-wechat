...
export async function refreshMessages(options: { reason?: RefreshReason; forceBottom?: boolean } = {}): Promise<void> {
  ...
  const messages = await api<MessageDto[]>(`/api/chats/${encodeURIComponent(chatId)}/messages?limit=100&offset=0`);
  if (seq !== messagesRequestSeq || chatId !== state.selectedChatId) return;
  ...
  bindLazyMediaImages((imageWasNearBottom) => {
    if (imageWasNearBottom) scrollToBottom();
    else restoreMessageScroll(captureMessageScroll());
  });
...
function renderConversationHeader(): void {
  const header = document.querySelector<HTMLElement>('[data-region="conversation-header"]');
...
function bindEvents(): void {
  document.addEventListener("click", (event) => {
    ...
    const chatButton = target.closest<HTMLButtonElement>("[data-chat]");
    if (chatButton) {
    ...
    if (action === "refresh") void refreshAll("initial");
    if (action === "login") state.status?.loggedIn ? void refreshAll("initial") : void startWechatLogin();
    if (action === "switch-login") void startWechatLogin(true);
    if (action === "open-service-folder") { state.serviceFolderOpen = true; renderChats(); return; }
    ...
    const download = target.closest<HTMLButtonElement>("[data-download]");
    if (download) {
  ...
  document.addEventListener("input", (event) => {
    if ((event.target as HTMLElement).id === "composer-input") autosizeComposer();
  });
  document.addEventListener("compositionstart", (event) => {
    if ((event.target as HTMLElement).id === "composer-input") state.composing = true;
  });
  document.addEventListener("compositionend", (event) => {
    if ((event.target as HTMLElement).id === "composer-input") state.composing = false;
  });
  document.addEventListener("keydown", (event) => {
    if ((event.target as HTMLElement).id === "composer-input" && event.key === "Enter" && !event.shiftKey && !state.composing) {
  ...
  document.addEventListener("paste", (event) => {
    const target = event.target as HTMLElement;
    if (target.id !== "composer-input" || !selectedChat()?.canSend) return;
